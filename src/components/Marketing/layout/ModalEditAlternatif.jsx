...
const ModalEditAlternatif = ({ loading, setLoading, id }) => {
  ...
  const updateData = async (data, id) => {
    const api = server;
    try {
      const response = await axios.put(`${api}/alternatif/${id}`, data);
  ...
  async function getData(id) {
    const api = server;
    try {
      const response = await axios.get(`${api}/alternatif/${id}`);
      setData({
        kode_alternatif: response.data.alternatif.kode_alternatif,
        bauran_promosi: response.data.alternatif.bauran_promosi,
        type: response.data.alternatif.type,
      });
      console.log(response);
    } catch (error) {
      console.error(error);
...
export default ModalEditAlternatif;
